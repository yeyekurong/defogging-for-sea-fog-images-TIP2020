function [img_dehazed, transmission, ind_dis] = non_local_dehazing(img_hazy, LR, gamma, dis, air_light)
...
%find the ind in sky regions
ind_dis = reshape(ind,h,w);
ind_h = ind_dis(1:round(h/3),:);   
...
bin_count_map   = reshape(bin_count(ind),h,w);   %pixel number
bin_eval_fun    = @(x) min(1, x/50);

% Calculate std - this is the data-term weight of Eq. (15)
K_std = accumarray(ind,radius(:),[n_points,1],@std);   %the standard of each haze-line
radius_std = reshape( K_std(ind), h, w);              
radius_eval_fun = @(r) min(1, 3*max(0.001, r-0.1));

% Calculate the the reliability from the depth field ****little trick
lam = adjust(LR(:,:,1));  
tr = transmission_estimation(ind==ind_ix);
tr = min(power(max(transmission_estimation/min(tr),1),dis),100); 
Imax = min(img_hazy,[],3);
N = boxfilter(ones(h,w),10);
Imax = boxfilter(Imax,10)./N;
ill = abs(max(air_light)-Imax)./max(air_light);
c2 = dis./(dis+exp(ill.*(lam-1).*tr));

transmission_estimation = 1-c2+c2.*transmission_estimation;
%Calculate the reliability from the std
...
leave_haze = 1.02; % leave a bit of haze for a natural look (set to 1 to reduce all haze)
for color_idx = 1:3
